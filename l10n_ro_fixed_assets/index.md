...
- **Reevaluare la valoare justă**: model dedicat `l10n.ro.asset.revaluation` cu wizard, accesibil din butonul „Reevaluare" al formularului activului; generează automat nota Dr 21x = Cr 105 (surplus) sau Dr 105/6813 = Cr 21x (depreciere); recalculează corect planul de amortizare de la data reevaluării încolo, pentru surplus și pentru depreciere deopotrivă (fix 19.0.1.3.1 — `action_post()` apela direct `_recompute_board()`, care doar calculează noile valori fără să șteargă liniile draft vechi și fără să creeze cele noi, astfel încât amortizarea viitoare continua tăcut la suma dinaintea reevaluării; acum apelează `compute_depreciation_board(date=self.date)`, care regenerează efectiv planul).
...
- `l10n.ro.asset.revaluation`: modelul de reevaluare a mijloacelor fixe (cont 105), cu monografie completă; `action_post()` recalculează planul de amortizare apelând `asset.compute_depreciation_board(date=self.date)` (fix 19.0.1.3.1 — anterior apela `_recompute_board()`, care nu regenera efectiv liniile).
