# Sale Withdrawal - Stock (localizat la `deltatech_sale_withdrawal_stock/index.md`)

- **Nume Tehnic:** `deltatech_sale_withdrawal_stock`
- **Versiune:** `19.0.0.2.0`
- **Cale:** [https://github.com/terrabit-solutions/bitshop/tree/19.0/deltatech_sale_withdrawal_stock](https://github.com/terrabit-solutions/bitshop/tree/19.0/deltatech_sale_withdrawal_stock)
- **Cale Locală:** `odoo-addons/bitshop/deltatech_sale_withdrawal_stock`
- **Ultima Ingestie:** 2026-09-22

#### 1. Sumar

Acest modul asigură continuarea operațională a retragerii dintr-un contract (renunțare de tip consumator) pe partea de stoc: atunci când un client renunță la un contract, marfa trebuie fie oprită din livrare, fie returnată. Modulul adaugă modul de execuție `stock_return` pentru `deltatech_sale_withdrawal`, disponibil în **Vânzări → Configurare → Setări → Retragere din Contract → Execuție Retragere**.
...
- `view_deltatech_sale_withdrawal_form_stock`: extinde formularul de retragere din `deltatech_sale_withdrawal` cu un buton statistic „Returns" (afișat doar când există transferuri de retur) și cu afișarea mutărilor anulate (`cancelled_move_ids`).
...
- [deltatech_sale_withdrawal](../deltatech_sale_withdrawal/index.md): modulul de bază pentru retragerea dintr-un contract, pe care acesta îl extinde cu execuția specifică stocului.
